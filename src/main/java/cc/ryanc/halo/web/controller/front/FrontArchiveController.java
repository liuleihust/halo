...
public class FrontArchiveController extends BaseController {
    ...
    /*
    @Cacheable标记的方法在执行后Spring Cache将缓存其返回结果 当标记在一个方法上时表示该方法是支持缓存的，当标记在一个类上时则表示该类所有的方法都是支持缓存的
    需要注意的是当一个支持缓存的方法在对象内部被调用时是不会触发缓存功能的
    value : 指定Cache 名称，表示当前方法的返回值是会被缓存在哪个Cache 上的。可以指定一个，也可以指定一个数组
    key: 用来指定Spring缓存方法的返回结果时对应的key ,没有指定时，采用默认策略生成key，

    @CacheEvict标记的方法会在方法执行前或者执行后移除Spring Cache中的某些元素。
    allEntries = true ： 表示删除所有内容
    beforeInvocation = true 表示在这个方法调用之前发生
     */
